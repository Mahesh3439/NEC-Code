...
import <%= componentName %>ViewModel, { I<%= componentName %>BindingContext } from './<%= componentName %>ViewModel';
...
export default class <%= componentClassName %> extends BaseClientSideWebPart <I<%= componentClassName %>Props> {
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: I<%= componentName %>BindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        ...
        template: require('./<%= componentName %>.template.html'),
        synchronous: false
      }
    );
}
